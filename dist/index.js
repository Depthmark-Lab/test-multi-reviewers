...
const CHECK_NAME = "review-integrity";
async function getCommitters(context, pr) {
...
async function getApprovals(context, pr) {
...
function buildCheckResult(committers, approvers) {
    const violatingApprovers = [];
    ...
    if (violatingApprovers.length > 0) {
        return {
            ...
            text: [
                ...
                ...violatingApprovers.map((u) => `- **@${u}** — committed and approved`),
                "\nA PR must be approved by someone who has **not** committed to the branch.",
                "\n### Committers on this PR",
                ...[...committers].map((u) => `- @${u}`),
                "\n### Current approvals",
                ...(approvers.size > 0
                    ? [...approvers.keys()].map((u) => violatingApprovers.includes(u)
...
async function runCheck(context, pr) {
    const app = context;
    const log = context.log || context.app?.log;
    log?.info(`Running review-integrity check for ${pr.owner}/${pr.repo}#${pr.pull_number} @ ${pr.head_sha}`);
    ...
    try {
        ...
        await context.octokit.checks.update({
            ...
            output: {
                title: result.conclusion === "success"
                    ? "Review integrity: OK"
        ...
        log?.info(`Check ${result.conclusion} for ${pr.owner}/${pr.repo}#${pr.pull_number}`);
    }
    catch (error) {
        await context.octokit.checks.update({
            owner: pr.owner,
...
function extractPrInfo(context) {
